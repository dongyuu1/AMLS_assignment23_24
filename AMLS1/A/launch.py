...
def get_data(cfg):
    ...
    if cfg.MODEL != 'CNN':
        ...
        # In this case, the lbp features are extracted first, then we extract hog features from them.
            ...
            print('Extracting HOG and LBP features of the training set')
            x_train = fe.extract_hog_and_lbp_features(x_train)
            print('Extracting HOG and LBP features of the validation set')
            x_val = fe.extract_hog_and_lbp_features(x_val)
            print('Extracting HOG and LBP features of the testing set')
...
def param_search(data, cfg):
    ...
    if cfg.MODEL == 'CNN':
        ...
        x_train = np.expand_dims(x_train, -1).repeat(3, axis=-1)
        x_test = np.expand_dims(x_test, -1).repeat(3, axis=-1)
...
def normal_train_test(data, cfg):
    ...
    if cfg.MODEL == 'CNN':
        # Expand the channel dimension of the data. N*H*W --> N*H*W*C
        x_train = np.expand_dims(x_train, -1).repeat(3, axis=-1)
        x_val = np.expand_dims(x_val, -1).repeat(3, axis=-1)
        x_test = np.expand_dims(x_test, -1).repeat(3, axis=-1)
