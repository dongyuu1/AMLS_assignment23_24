...
class CNN:
    def __init__(self, config):
        ...
        self.clf = models.resnet18(pretrained=self.cfg.CNN.PRETRAINED, num_classes=2)
        # Transfer the model to the corresponding device (cpu or cuda:0)
        self.clf.to(device=self.cfg.CNN.DEVICE)

    def train(self, train_data, val_data):
        if not os.path.exists('./A/weights.pt'):
            ...
            torch.save(self.clf, './A/weights.pt')
        else:
    ...
    def random_search(self, data):
        ...
        param_grid = {'optimizer': ['sgd', 'adam', 'adamw'],
        ...
        for i in range(search_iter):
            ...
            classifier = models.resnet18(pretrained=False, num_classes=2)
    ...
    def train_loop(self, classifier, train_dataset, val_dataset, optimizer, loss_fn, scheduler):
        ...
        for e in range(1, self.cfg.CNN.EPOCH + 1):
            ...
            for x, y in train_loader:
                ...
                y = F.one_hot(y).to(device=self.cfg.CNN.DEVICE, dtype=torch.float32)
    ...
    def test(self, test_data):
        ...
        self.clf = torch.load('./A/weights.pt')
    ...
    def test_loop(self, test_dataset, classifier):
        ...
        total_y_pred = torch.zeros([len(test_dataset), self.cfg.CNN.NUM_CLASS],
                                   dtype=torch.float).to(device=self.cfg.CNN.DEVICE)
        total_y = torch.zeros([len(test_dataset), self.cfg.CNN.NUM_CLASS],
                              dtype=torch.float).to(device=self.cfg.CNN.DEVICE)
